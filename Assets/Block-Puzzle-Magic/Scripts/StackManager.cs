...
public class StackManager : Singleton<StackManager> 
{
	...
	public static int RwdScorecount;
	...
	{
		GameObject currentWindow = WindowStack.Peek ();
		Debug.Log("CurrenrtWindow1 " + currentWindow);
	...
	public void RestartRwdGamePlay()
	{
		GameObject currentWindow = WindowStack.Peek();
		Debug.Log("CurrenrtWindow" + currentWindow);
		if (currentWindow != null)
		{
			Debug.Log("CurrenrtWindow**" + currentWindow);
			if ( currentWindow.name == "Rewardvideo")
			{
				if (currentWindow.OnWindowRemove() == false)
				{
					Destroy(currentWindow);
			...
			WindowStack.Pop();
			Destroy(PeekWindow());
			Invoke("LoadRwdGamePlay", 0.1F);
        }
	}

	void LoadRwdGamePlay()
	{
		SpawnUIScreen("GamePlay", true);
		GameController.AlertCount = 1;
		RwdScorecount = 1;
